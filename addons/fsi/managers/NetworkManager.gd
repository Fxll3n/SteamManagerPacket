...
func _process(delta: float) -> void:
	if not Steam.isSteamRunning():
		print("[NetworkManager] Steam not running.")
		return
...
func create_lobby() -> void:
	if lobby_id != 0:
		push_warning(str("[NetworkManager] User is already in a lobby:\t%s" % lobby_id))
		return
	is_host = true
...
func read_p2p_packet() -> void:
	...
	if packet_size > 0:
		print("[NetworkManager] Recieved a packet.")
		var this_packet: Dictionary = Steam.readP2PPacket(packet_size)
		#var packet_sender: int = this_packet["steam_remote_id"]
		var packet_code: PackedByteArray = this_packet["data"]
		var readable_data: Dictionary = bytes_to_var(packet_code) as Dictionary
		
		if readable_data.has("tag"):
			print("[NetworkManager] Packet contains a tag.")
			recieved_packet.emit(readable_data)
			match readable_data["tag"]:
				"handshake":
					print("[NetworkManager] Acknowledged handshake from %s (AKA %s)" % [readable_data["steam_id"], readable_data["username"]])
					print("[NetworkManager] %s has joined the lobby." % readable_data["username"])
...
func make_p2p_handshake() -> void:
	...
	print("[NetworkManager] Initiated Handshake.")
#endregion
#region SIGNALS
func _on_lobby_created(connection: int, this_lobby_id: int) -> void:
	if connection != 1:
		push_warning("[NetworkManager] Lobby creation failed with error code:\t%s" % connection)
		return
	...
	print("[NetworkManager] Lobby created with id:\t%s" % lobby_id)
...
func _on_lobby_joined(this_lobby_id: int, _permisions: int, _locked: bool, response: int) -> void:
	if response != Steam.CHAT_ROOM_ENTER_RESPONSE_SUCCESS:
		...
		return
	lobby_id = this_lobby_id
	print("[NetworkManager] Sucessfully joined lobby:\t%s" % this_lobby_id)
	get_lobby_members()
	make_p2p_handshake()
